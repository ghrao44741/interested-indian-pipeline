...
def _write_v2_approval(project_dir: Path, approver: str, confirmation: str) -> Path:
    ...
    regression-test generation_gate.require_generation_ready(), unchanged
    and structurally isolated from the v3 cutover."""
    ...
    if _route_failures.unresolved(project_dir):
    ...
    record = {
        ...
        "approved_at": _dt.now(_tz.utc).isoformat(timespec="seconds"),
...
def s10_mismatch_blocks():
    ...
    try:
        with World(root):
            _write_v2_approval(proj, "R", _v2_confirmation_phrase(proj.name, p["plan_id"]))
