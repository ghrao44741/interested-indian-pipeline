...
- (this session's stitch/caption/manifest fixes — see commit log for the actual hash)

### Pending / handoff
- **Script draft awaiting review** — `pilot_neet_scandal/script_the_exam_scandal_a_cockroach_party_and_a_ministers_res_DRAFT.txt` has the first-person rewrite + million removal; regenerating narration from it (if approved) cascades through split → images again.
- Once the script is finalized, the whole episode needs a full re-run from narration onward — the current stitched video is on the pre-rewrite script.
- BGM decision (Prism) is per-project (`pilot_neet_scandal/bgm.mp3`) — if it works well, consider whether to promote it to the new global default, or keep `common/bgm/default.mp3` (Aeon) as-is for other projects.
...
- The A/V-sync bug (`apad` fix) was long-standing in `stitch_video_longform.py` — worth re-checking any previously "finished" episode (ep01, test_script) for the same drift if they're revisited.
