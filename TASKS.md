...
- [x] **#18** (Task 2B-A) Evaluation defaults should come from the Channel Pack, not module
  `DEFAULT_*` constants — `generate_source_audio.py`'s evaluation path (an omitted
  `--provider`/`--voice`) used to fall back to `DEFAULT_PROVIDER`/`DEFAULT_VOICE_*`, read from
  the root `channel_config.json` at import time — the generated legacy adapter for
  `interested_indian` specifically, not the loaded Channel Pack's own `voice.working_default`.
  A second channel with no legacy adapter would have gotten Interested Indian's defaults
  instead of its own, silently. Flagged during the second Task 2A follow-up review (2026-08-02)
  alongside the `--preview`/full-profile-forwarding/cloudtts-fallback/split-containment fixes in
  `e8744f7` and `5fdc828` — explicitly scoped OUT of that micro-fix at the time. Fixed in
  `ffc15b4` (Task 2B-A, 2026-08-02): `voice.working_default` is now provider-discriminated in
  the schema (identical per-provider settings shape to `approved_profile`), and a new
  `_resolve_evaluation_voice()` resolves evaluation dispatch from it — refusing before any
  credential/client/`mkdir` if the working default is null or an explicit `--provider` conflicts
  with it, with CLI overrides layered on top per field. Evaluation sidecars now record
  `channel_id` and the complete effective settings actually used, truthfully naming both
  requested and actual provider when the `gemini_cloudtts → gemini` fallback fires. Task 2B's
  remaining scope (semantic routing, `visual_routes.json`, deterministic renderers) is
