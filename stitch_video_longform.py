...
try:
    ...
    from generate_srt import generate_srt, generate_ass
...
def build_clip_from_video(scene_id: str, video_path: str, audio_path: str,
    ...
    -af apad pads the audio with silence out to the requested -t duration.
    Without it, ffmpeg just lets the audio input run out ~0.5s before the
    video does (video is generated by an infinite loop, audio isn't) — each
    clip's audio track ends up ~0.5s shorter than its video track, and that
    gap compounds across every scene when clips are concatenated, so by the
    end of a long episode the real narration audio runs tens of seconds
    behind the picture despite each individual clip looking fine in isolation.
    """
...
def build_clip_from_image(scene_id: str, image_path: str, audio_path: str,
    ...
    cmd = [
        ...
        "-pix_fmt", "yuv420p",
        "-vf", vf_chain,
        "-af", "apad",  # pad audio with silence to fill total_duration — see build_clip_from_video's docstring
...
# ═══════════════════════════════════════════════════════════════════════════════
# CAPTION BURN (ASS — landscape-appropriate, bottom-third)
# ═══════════════════════════════════════════════════════════════════════════════

def burn_ass_captions(video_path: str, ass_path: str, output_path: str):
    """
    Burn captions into the video using ffmpeg's subtitles filter against a
    real .ass file (see generate_srt.py's generate_ass()).

    A plain .srt has no PlayResX/PlayResY of its own, so when burned directly
    ffmpeg/libass silently assumed a small default reference resolution and
    scaled our wrapping width and MarginV unpredictably — captions ended up
    wrapping into far more lines than intended, or sitting much higher on
    screen than the MarginV value implied. The .ass file declares
    PlayResX=1920/PlayResY=1080 explicitly, so what generate_ass() sets is
    exactly what renders — no more guessing.

    The style itself (BorderStyle=3 + BackColour = solid box behind the
    text) is baked into the .ass file's own [V4+ Styles] section, not
    passed here — no fixed vertical band is ever guaranteed clear of the
    artwork itself (characters drawn large/centered, especially after Ken
    Burns zoom, can reach into the bottom-third caption zone in any given
    scene), so the caption needs to stay legible over arbitrary art rather
    than relying on a "safe" empty area that doesn't reliably exist.
    """
...
def main():
    ...
    print(f"\n{'═' * 55}")
    print("STEP 3/4 — GENERATE CAPTIONS")
    print(f"{'═' * 55}\n")
    generate_srt(project_dir)  # kept for uploading as a YouTube subtitle track
    generate_ass(project_dir)  # used for the burn — see burn_ass_captions() docstring

    # ── Step 4: Burn captions ─────────────────────────────────────────────
    print(f"\n{'═' * 55}")
    print("STEP 4/4 — BURN CAPTIONS (bottom-third)")
    print(f"{'═' * 55}\n")
    ...
    if os.path.exists(ass_path):
        print(f"Burning captions from {ass_path}...")
        burn_ass_captions(final_video, ass_path, captioned_out)
        print(f"✓ Captioned: {captioned_out}")
    else:
        print(f"⚠️  ASS not found at {ass_path} — skipping burn")
